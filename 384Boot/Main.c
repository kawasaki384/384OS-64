    ...
    EFI_SYSTEM_TABLE *system_table)
{
    ...
    EFI_FILE_PROTOCOL *psf_font;
/*********************************************************************************/

    PSF1_FONT *finishedFont;

    status = root_dir->Open(
        root_dir, &psf_font, L"font.psf",
        EFI_FILE_MODE_READ, 0);
    if(EFI_ERROR(status))
    {
        Print(L"fontfile not found");
        Halt();
    }
    else
    {
        Print(L"FontFile_found\n");
...
#define PSF1_MODE512  0x01
        ...
        psf_font->Read(psf_font, &size, fontHeader);
        ...
        if(fontHeader->mode == PSF1_MODE512) {
            glyphBufferSize = fontHeader->charsize * 512;
        }
        void *glyphBuffer;
        {
            psf_font->SetPosition(psf_font, sizeof(psf1_header));

            system_table->BootServices->AllocatePool(EfiLoaderData, glyphBufferSize, (void **)&glyphBuffer);
            psf_font->Read(psf_font, &glyphBufferSize, glyphBuffer);
        }
    ...
    typedef void EntryPointType(const KAGraphic*,PSF1_FONT*,struct MemoryMap*);
    EntryPointType *entry_point = (EntryPointType *)entry_addr;
    entry_point(&GraphicKernelArg,finishedFont,&memmap);

    Print(L"All done %d %d\n",memmap.map_size,memmap.map_key);
